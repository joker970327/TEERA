...
int PAIR_BLS12383_nbits(BIG_384_58 n3, BIG_384_58 n)
{
    BIG_384_58 x;
...
/* GLV method */
static void glv(BIG_384_58 u[2], BIG_384_58 ee)
{
    BIG_384_58 q;
#if PAIRING_FRIENDLY_BLS12383==BN_CURVE
    int i, j;
    BIG_384_58 v[2], t;
    DBIG_384_58 d;
    BIG_384_58_rcopy(q, CURVE_Order_BLS12383);

    for (i = 0; i < 2; i++)
    {
        BIG_384_58_rcopy(t, CURVE_W_BLS12383[i]);
        BIG_384_58_mul(d, t, ee);
        BIG_384_58_ctddiv(v[i],d,q,BIG_384_58_nbits(t));
        BIG_384_58_zero(u[i]);
    }
    BIG_384_58_copy(u[0], ee);
    for (i = 0; i < 2; i++)
        for (j = 0; j < 2; j++)
        {
            BIG_384_58_rcopy(t, CURVE_SB_BLS12383[j][i]);
            BIG_384_58_modmul(t, v[j], t, q);
            BIG_384_58_add(u[i], u[i], q);
            BIG_384_58_sub(u[i], u[i], t);
            BIG_384_58_ctmod(u[i],q,1);
        }
...
// -(x^2).P = (Beta.x,y)
    ...
    bd=BIG_384_58_nbits(q)-BIG_384_58_nbits(x2); // fixed
    BIG_384_58_copy(u[0], ee);
    BIG_384_58_ctmod(u[0], x2, bd);
    BIG_384_58_copy(u[1], ee);
    BIG_384_58_ctsdiv(u[1], x2, bd);
...
static void gs(BIG_384_58 u[4], BIG_384_58 ee)
{
    int i;
    BIG_384_58 q;
#if PAIRING_FRIENDLY_BLS12383==BN_CURVE
    ...
    for (i = 0; i < 4; i++)
    {
        BIG_384_58_rcopy(t, CURVE_WB_BLS12383[i]);
        BIG_384_58_mul(d, t, ee);
        BIG_384_58_ctddiv(v[i],d,q,BIG_384_58_nbits(t));
        BIG_384_58_zero(u[i]);
    }
    ...
    for (i = 0; i < 4; i++)
        for (j = 0; j < 4; j++)
        {
            BIG_384_58_rcopy(t, CURVE_BB_BLS12383[j][i]);
            BIG_384_58_modmul(t, v[j], t, q);
            BIG_384_58_add(u[i], u[i], q);
            BIG_384_58_sub(u[i], u[i], t);
            BIG_384_58_ctmod(u[i],q,1);
    ...
    BIG_384_58_rcopy(x, CURVE_Bnx_BLS12383);
    BIG_384_58_copy(w, ee);
    bd=BIG_384_58_nbits(q)-BIG_384_58_nbits(x); // fixed

    for (i = 0; i < 3; i++)
    {
        BIG_384_58_copy(u[i], w);
        BIG_384_58_ctmod(u[i],x,bd);
        BIG_384_58_ctsdiv(w,x,bd);
    }
...
void PAIR_BLS12383_G1mul(ECP_BLS12383 *P, BIG_384_58 e)
{
    BIG_384_58 ee,q;
...
void PAIR_BLS12383_G2mul(ECP2_BLS12383 *P, BIG_384_58 e)
{
    BIG_384_58 ee,q;
...
/* f=f^e */
void PAIR_BLS12383_GTpow(FP12_BLS12383 *f, BIG_384_58 e)
{
    BIG_384_58 ee,q;
...
int PAIR_BLS12383_GTmember(FP12_BLS12383 *m)
{
    ...
    BIG_384_58_zero(x); BIG_384_58_inc(x,6);
    FP12_BLS12383_pow(&t,&t,x);
#else
#if SIGN_OF_X_BLS12383==NEGATIVEX
